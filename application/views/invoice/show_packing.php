...
<div class="container" style="margin-top: 100px;">
    ...
    foreach($orders as $order): 
        ...
        if (!empty($order->shipping_address) || !empty($order->shipping_address_line2) || !empty($order->shipping_address_line3) || !empty($order->shipping_address_line4) || !empty($order->shipping_address_city) || !empty($order->shipping_address_postcode)) {
            ...
            $shipping = trim($order->delivery_address .' '. $order->delivery_address_line2 .' '. $order->delivery_address_line3 .' '. $order->delivery_address_line4 .' '. $order->delivery_address_city .' '. $order->delivery_address_postcode);
        }
        
        // Check if the company name or shipping address has changed
        if ($order->company_name != $current_company || $shipping != $current_shipping):
